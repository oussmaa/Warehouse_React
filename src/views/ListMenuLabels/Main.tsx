...
function Main() {
  ...
  const [formattedMenu, setFormattedMenu] = useState<MenuLabel[]>([]);
  const navigate = useNavigate();
  const location = useLocation();
  const menuId = location.state?.idmenu;
  ...
  const fetchDataAndUpdateMenu = async () => {
    try {
      console.log(33)
        ...
        console.log(ApiUrls.GETLISTLABELBYID)

      const menuData = await apiService.GetListMenuLabel(ApiUrls.GETLISTLABELBYID+menuId);
            ...
            <tbody>
              {formattedMenu.map((menu, keyy) => (
                <tr key={keyy} className="intro-x">
                  ...
                  <td>
                    <div style={{ backgroundColor: menu.icon }} className="text-slate-500 text-xs whitespace-nowrap mt-0.5">
                      {menu.pathname}
                    </div>
                  </td>                
                  <td className="table-report__action w-56">
                    <div className="flex justify-center items-center">
                      <a className="flex items-center mr-3"   onClick={() => navigatetoaddSubbmenulabel(menu.id)}  > 
